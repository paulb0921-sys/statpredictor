...
# Try to import sklearn, fallback with clear message
try:
    from sklearn.model_selection import train_test_split
    from sklearn.ensemble import RandomForestRegressor
    from sklearn.metrics import mean_absolute_error, mean_squared_error
except ImportError:
    st.error('Scikit-learn is not installed. Please run `pip install scikit-learn` in this environment.')
    st.stop()
...
def train_model(X, y, use_lightgbm=True):
    ...
    return {'model_type': wrapper[0], 'model_obj': wrapper[1], 'feature_names': list(X.columns)}
